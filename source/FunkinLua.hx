...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('genericSongName', PlayState.genericSong);
		set('startedCountdown', false);
		set('trueEctospasm', PlayState.trueEctospasm);
		set('firstTry', PlayState.firstTry);
		...
		// Character shit
		set('boyfriendName', PlayState.bfVersion);
		set('dadName', PlayState.foeVersion);
		set('gfName', PlayState.gfVersion);
		...
		set('noteQuantization', ClientPrefs.noteQuantization);
		...
		set('screenShake', ClientPrefs.screenShake);
		set('windowShake', ClientPrefs.windowShake);
		set('ghostTrails', ClientPrefs.ghostTrails);
		set('motion', ClientPrefs.motion);
		set('optimize', ClientPrefs.optimize);
		set('backgroundLevel', ClientPrefs.background);
		set('modcharts', ClientPrefs.modcharts);
		set('cacheStory', ClientPrefs.cacheStory);
		set('randomMode', PlayState.randomMode);

		set('windowX', Application.current.window.x);
		set('windowY', Application.current.window.y);
		...
		Lua_helper.add_callback(lua, "removeFromTracked", function(tag:String) {
			var asset:String = tag;
			var spr:FlxSprite = getObjectDirectly(tag);
			if(spr != null) {
				asset = spr.graphic.assetsKey;
			}

			return Paths.localTrackedAssets.remove(asset);
		});

		Lua_helper.add_callback(lua, "removeSoundFromTracked", function(tag:String) {
			var asset = Paths.getSoundPath(tag);
		...
		Lua_helper.add_callback(lua, "imageExists", function(image:String, ?library:String) {
			return Paths.imageExists(image, library);
		});
		...
		Lua_helper.add_callback(lua, "cameraShake", function(camera:String, intensity:Float, duration:Float) {
			var camera = cameraFromString(camera);
			camera.shake(intensity, duration);
			//@:privateAccess camera.updateFlashSpritePosition();
			//@:privateAccess camera.updateShake(0);
		});
		...
		Lua_helper.add_callback(lua, "cameraFlash", function(camera:String, color:String, duration:Float,forced:Bool) {
			...
			var camera = cameraFromString(camera);
			camera.flash(colorNum, duration,null,forced);
		...
		Lua_helper.add_callback(lua, "cameraFade", function(camera:String, color:String, duration:Float,forced:Bool) {
			...
			if(!color.startsWith('0x')) colorNum = Std.parseInt('0xff' + color);
			var camera = cameraFromString(camera);
			camera.fade(colorNum, duration,false,null,forced);
			@:privateAccess camera.updateFade(0);
		});
		...
		Lua_helper.add_callback(lua, "objectStopAnimation", function(obj:String) {
			if(PlayState.instance.modchartSprites.exists(obj)) {
				PlayState.instance.modchartSprites.get(obj).animation.stop();
				return;
			}
		...
		Lua_helper.add_callback(lua, "setWrathShader", function(obj:String, preset:String, angleOpt:Int = 17) {
			var spr:FlxSprite = getObjectDirectly(obj);
			if(!ClientPrefs.shaders) return;
			if(!ClientPrefs.wrathShader) return;
			spr.shader = new WrathShader(WrathShader.fromString(preset), angleOpt);
		});

		Lua_helper.add_callback(lua, "setWrathChromShader", function(obj:String, angleOpt:Int = 17) {
			var spr:FlxSprite = getObjectDirectly(obj);
			if(!ClientPrefs.shaders) return;
			if(!ClientPrefs.wrathShader) return;
			spr.shader = new WrathShaderChromatic(angleOpt);
		});

		Lua_helper.add_callback(lua, "removeShader", function(obj:String) {
			var spr:FlxSprite = getObjectDirectly(obj);
			if(!ClientPrefs.shaders || spr.shader == null) return;
			if (spr.shader != null) spr.shader = null;
		});

		Lua_helper.add_callback(lua, "setChromaticShader", function(obj:String) {
			var spr:FlxSprite = getObjectDirectly(obj);
			if(!ClientPrefs.shaders) return;
			if(!ClientPrefs.chromatic) return;
			spr.shader = new ChromaticAberrationShader();
		});

		Lua_helper.add_callback(lua, "setColorSwapShader", function(obj:String, hue:Float = 0, sat:Float = 0, bri:Float = 0) {
			var spr:FlxSprite = getObjectDirectly(obj);
			//if(!ClientPrefs.shaders) return;
			var colorSwap = new ColorSwap();
			colorSwap.hue = hue / 360;
			colorSwap.saturation = sat / 100;
			colorSwap.brightness = bri / 100;
			spr.shader = colorSwap.shader;
		});

		Lua_helper.add_callback(lua, "setAmongUsShader", function(obj:String, red:String = "", green:String = "", blue:String = "") {
			var spr:FlxSprite = getObjectDirectly(obj);
			//if(!ClientPrefs.shaders) return;
			var shader = new shaders.AmongUsShader();

			if(red != "") shader.red = FlxColor.fromString(red);
			if(green != "") shader.green = FlxColor.fromString(green);
			if(blue != "") shader.blue = FlxColor.fromString(blue);
