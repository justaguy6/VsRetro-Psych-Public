...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		if(difficultyChoices.length < 2) menuItemsOG.remove('Change Difficulty'); //No need to change difficulty if there is only one!
		...
		var levelInfo = new FlxFixedText(20, 15, 0, "", 32);
		levelInfo.text += (PlayState.trueEctospasm ? "True ": "") + PlayState.SONG.song;
		levelInfo.scrollFactor.set();
		levelInfo.setFormat(Paths.font("vcr.ttf"), 32);
		levelInfo.updateHitbox();
		add(levelInfo);

		var levelDifficulty = new FlxFixedText(20, 15 + 32, 0, "", 32);
		...
		var blueballedTxt = new FlxFixedText(20, 15 + 64, 0, "", 32);
		...
		if(PlayState.chartingMode) {
			var chartingText = new FlxFixedText(20, 15 + 101, 0, "CHARTING MODE", 32);
	...
	function updateSkipTextStuff()
	{
		...
		if(skipTimeTracker == null) return;
